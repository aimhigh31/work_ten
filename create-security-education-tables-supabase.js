...
async function createSecurityEducationTables() {
  ...
  try {
    ...
    const { error: mainTableError } = await supabase.rpc('exec_sql', { sql: createMainTableSQL });
    if (mainTableError) {
    ...
    const { error: curriculumError } = await supabase.rpc('exec_sql', { sql: createCurriculumTableSQL });
    if (curriculumError) {
    ...
    const { error: attendeeError } = await supabase.rpc('exec_sql', { sql: createAttendeeTableSQL });
    if (attendeeError) {
...
async function createSampleCurriculum(educationId) {
...
async function createSampleAttendees(educationId) {
  const attendees = [
    {
      education_id: educationId,
      user_name: '박디자이너',
