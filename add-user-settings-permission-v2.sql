-- ========================================
-- 사용자설정 페이지 접근 권한 추가 (V2)
-- 대상: jsan (jaesikan@nexplus.co.kr) - ROLE-25-ADMIN
-- 컬럼명: menu_page, menu_url (확인됨)
-- ========================================
...
BEGIN
  ...
  -- 2. 사용자설정 메뉴 ID 조회 (정확한 컬럼명 사용)
  SELECT id INTO v_menu_id
  FROM admin_systemsetting_menu
  WHERE menu_url = '/admin-panel/user-settings'
     OR menu_page LIKE '%사용자설정%'
     OR menu_page LIKE '%사용자 설정%';
...
WHERE r.role_code = 'ROLE-25-ADMIN'
  AND (m.menu_url = '/admin-panel/user-settings' OR m.menu_page LIKE '%사용자설정%')
