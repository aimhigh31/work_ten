...
async function executeSqlFile() {
  ...
  try {
    ...
    // SQL 파일 읽기
    const sqlFilePath = path.join(__dirname, 'add_changed_field.sql');
    const sqlContent = fs.readFileSync(sqlFilePath, 'utf8');

    console.log('📝 changed_field 컬럼 추가 SQL 실행 중...');
    ...
    for (let i = 0; i < queries.length; i++) {
      ...
      if (query) {
        console.log(`실행 중 (${i + 1}/${queries.length}): ${query.substring(0, 50)}...`);
    ...
    console.log('✅ changed_field 컬럼이 추가되었습니다!');
