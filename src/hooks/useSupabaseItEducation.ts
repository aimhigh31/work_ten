...
export function useSupabaseItEducation() {
  ...
  const getItEducationById = useCallback(async (id: number): Promise<ItEducationData | null> => {
    ...
    try {
      const { data, error } = await supabase.from('it_education_data').select('*').eq('id', id).eq('is_active', true).single();
  ...
  // IT교육 데이터 추가
  const addItEducation = useCallback(
    async (item: Omit<ItEducationData, 'id' | 'created_at' | 'updated_at'>): Promise<ItEducationData | null> => {
      setLoading(true);
      setError(null);
      ...
      try {
        ...
        const { data, error } = await supabase.from('it_education_data').insert([insertData]).select().single();
  ...
  const updateItEducation = useCallback(async (id: number, updates: Partial<ItEducationData>): Promise<ItEducationData | null> => {
    ...
    try {
      ...
      const { data, error } = await supabase.from('it_education_data').update(updateData).eq('id', id).select().single();
