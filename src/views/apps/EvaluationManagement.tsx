...
// Project imports
import EvaluationTable from 'views/apps/EvaluationTable';
...
export default function EvaluationManagement() {
  ...
  // 인사평가 코드 생성 함수 (HR-EVA-25-001 형식)
  const generateEvaluationCode = React.useCallback(async (): Promise<string> => {
    try {
      ...
      return `HR-EVA-${currentYear}-${nextNumber}`;
    } catch (error) {
      console.error('코드 생성 실패:', error);
      // 실패 시 타임스탬프 기반 코드 생성
      const year = new Date().getFullYear().toString().slice(-2);
      const random = Math.floor(Math.random() * 1000).toString().padStart(3, '0');
      return `HR-EVA-${year}-${random}`;
    }
  }, [evaluationDataList]);
  ...
  // 평가유형 목록 가져오기 (GROUP043) - { subcode, subcode_name } 형태
  const evaluationTypesMap = React.useMemo(() => {
    const types = masterCodes
      .filter((item) => item.codetype === 'subcode' && item.group_code === 'GROUP043' && item.is_active)
      .sort((a, b) => a.subcode_order - b.subcode_order);
    console.log('🔍 EvaluationManagement - GROUP043 평가유형 목록:', types);
    return types;
  }, [masterCodes]);
  ...
  // 관리분류 목록 가져오기 (GROUP040)
  const managementCategoryMap = React.useMemo(() => {
    const categories = masterCodes
      .filter((item) => item.codetype === 'subcode' && item.group_code === 'GROUP040' && item.is_active)
      .sort((a, b) => a.subcode_order - b.subcode_order);
    console.log('🔍 EvaluationManagement - GROUP040 관리분류 목록:', categories);
    return categories;
  }, [masterCodes]);

  // subcode → subcode_name 변환 (관리분류용)
  const getManagementCategoryName = React.useCallback((subcode: string) => {
    const found = managementCategoryMap.find(item => item.subcode === subcode);
    return found ? found.subcode_name : subcode;
  }, [managementCategoryMap]);
  ...
  const evaluationTableData = useMemo(() => {
    return evaluationDataList.map((item, index) => {
      // HR-EVA-25-001 형식 코드 생성
      const year = item.created_at
        ? new Date(item.created_at).getFullYear().toString().slice(-2)
        : new Date().getFullYear().toString().slice(-2);
      const serialNumber = String(item.id || index + 1).padStart(3, '0');
      const code = `HR-EVA-${year}-${serialNumber}`;

      return {
        id: item.id || Date.now() + index,
        no: evaluationDataList.length - index,
        registrationDate: item.created_at ? new Date(item.created_at).toISOString().split('T')[0] : new Date().toISOString().split('T')[0],
        code: code,
  ...
  // evaluationTableData가 변경될 때 inspections 상태 업데이트
  useEffect(() => {
    console.log('📊 evaluationTableData 변경 감지:', evaluationTableData.length, '개 항목');
    setInspections(evaluationTableData);
  ...
  // 변경로그 Hook (전체 인사평가의 변경 이력)
  const { logs: dbChangeLogs, loading: changeLogsLoading, fetchChangeLogs } = useSupabaseChangeLog('hr_evaluation');
              ...
              >
                <EvaluationTable
                  ...
                  setEvaluations={(newEvaluations) => {
                    console.log('평가 데이터 업데이트:', newEvaluations);
                  }}
