        ...
        <Table size="small">
          <TableHead>
            <TableRow sx={{ backgroundColor: theme.palette.grey[50] }}>
              ...
              <TableCell sx={{ fontWeight: 600, width: 180 }}>점검내용</TableCell>
            ...
            {paginatedLogs.map((log, index) => (
              ...
              >
                ...
                <TableCell>
                  <Typography variant="body2" sx={{ fontSize: '13px' }}>
                    {(() => {
                      const inspection = inspections.find((inspection) => inspection.code === log.target);
                      return inspection?.inspectionTitle || log.description.split(' - ')[0] || '점검내용 없음';
                    })()}
                  </Typography>
                </TableCell>
                <TableCell>
...
export default function InspectionManagement() {
  ...
  const [changeLogs, setChangeLogs] = useState<ChangeLog[]>([
    ...
    },
    {
      ...
      action: '새 점검 생성',
      target: 'TASK-24-011',
      description: '모바일 앱 UI/UX 개선 점검 신규 등록'
    },
    {
      id: 3,
    ...
    },
    {
      ...
      action: '점검 삭제',
      target: 'TASK-24-007',
      description: '중복된 데이터베이스 최적화 점검 삭제'
    }
  ]);
    ...
    try {
      if (originalInspection) {
        ...
        if (result) {
          ...
          if (originalInspection.assignee !== updatedInspection.assignee) {
            changes.push(`담당자: ${originalInspection.assignee} → ${updatedInspection.assignee}`);
          }

          if (changes.length > 0) {
            addChangeLog('점검 수정', updatedInspection.code, changes.join(', '), updatedInspection.team);
          }
        ...
        if (result) {
          ...
          addChangeLog(
            '점검 생성',
            updatedInspection.code,
            `새로운 점검이 생성되었습니다: ${updatedInspection.inspectionContent}`,
