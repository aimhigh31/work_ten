...
import DepartmentManagementTable from 'views/apps/DepartmentManagementTable';
import UserManagementTable from 'views/apps/UserManagementTable';
import RoleManagementTable from 'views/apps/RoleManagementTable';
...
// Icons
import { TableDocument, Chart, Calendar, Element, DocumentText, Building, People, Profile } from '@wandersonalwes/iconsax-react';
...
function KanbanView({ selectedYear, selectedTeam, selectedStatus, selectedAssignee, tasks, setTasks, addChangeLog }: KanbanViewProps) {
        ...
        <div className="card-footer">
          <div className="assignee-info">
            <img
              ...
              src={assigneeAvatars[task.assignee] || '/assets/images/users/default-avatar.png'}
              alt={task.assignee || 'Unassigned'}
              onError={(e) => {
                (e.target as HTMLImageElement).src = '/assets/images/users/default-avatar.png';
              }}
            />
...
// 업무관리와 동일한 구조의 간단한 변경로그 뷰
function SimpleChangeLogView({
  changeLogs,
  tasks,
  page,
  rowsPerPage,
    ...
    <Box sx={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
      ...
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mt: 2, flexShrink: 0 }}>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
          ...
          <Select value={rowsPerPage} onChange={(e) => onRowsPerPageChange(Number(e.target.value))} size="small" sx={{ minWidth: 70 }}>
        ...
        <Table size="small">
          <TableHead>
            <TableRow sx={{ backgroundColor: theme.palette.grey[50] }}>
              <TableCell sx={{ fontWeight: 600, width: 50 }}>NO</TableCell>
              <TableCell sx={{ fontWeight: 600, width: 130 }}>변경시간</TableCell>
              <TableCell sx={{ fontWeight: 600, width: 80 }}>메뉴</TableCell>
              <TableCell sx={{ fontWeight: 600, width: 100 }}>페이지</TableCell>
              <TableCell sx={{ fontWeight: 600, width: 100 }}>페이지코드</TableCell>
              <TableCell sx={{ fontWeight: 600, width: 180 }}>제목</TableCell>
              <TableCell sx={{ fontWeight: 600, width: 120 }}>변경분류</TableCell>
              <TableCell sx={{ fontWeight: 600, width: 220 }}>변경 세부내용</TableCell>
              <TableCell sx={{ fontWeight: 600, width: 90 }}>팀</TableCell>
              <TableCell sx={{ fontWeight: 600, width: 90 }}>담당자</TableCell>
            </TableRow>
          </TableHead>
                ...
                <TableCell>
                  <Typography variant="body2" sx={{ fontSize: '13px' }}>
                    {log.title ||
                      (() => {
                        const task = tasks.find((task) => task.code === log.target);
                        return task?.workContent || log.description.split(' - ')[0] || '제목 없음';
                      })()}
                  </Typography>
                </TableCell>
                <TableCell>
...
export default function TaskManagement() {
  ...
  // 권한 관리
  const { canViewCategory, canReadData, canCreateData, canEditOwn, canEditOthers, loading: permissionLoading } = useMenuPermission('/admin-panel/user-settings');
  ...
  // 모든 메뉴의 변경로그를 통합해서 가져오는 함수
  const getAllChangeLogs = () => {
    // 실제 구현에서는 localStorage나 전역 상태에서 각 메뉴의 변경로그를 수집
    const allChangeLogs: ChangeLog[] = [];

    // 관리자메뉴 변경로그
    const adminLogs = JSON.parse(localStorage.getItem('adminChangeLogs') || '[]');

    // 메인메뉴 변경로그 (업무관리, 비용관리 등)
    const taskLogs = JSON.parse(localStorage.getItem('taskChangeLogs') || '[]');
    const costLogs = JSON.parse(localStorage.getItem('costChangeLogs') || '[]');

    // 기획메뉴 변경로그
    const planningLogs = JSON.parse(localStorage.getItem('planningChangeLogs') || '[]');

    // IT메뉴 변경로그
    const itLogs = JSON.parse(localStorage.getItem('itChangeLogs') || '[]');

    // 보안메뉴 변경로그
    const securityLogs = JSON.parse(localStorage.getItem('securityChangeLogs') || '[]');
    ...
    const combinedLogs = [
      ...
      },
      {
        ...
        action: '완료일 수정',
        target: 'COST-24-005',
        description: '로고 디자인 비용 완료 예정일을 2024-12-20으로 수정',
        menuType: '메인메뉴',
        page: '비용관리',
        pageCode: 'COST-24-005',
        title: '로고 디자인 작업 비용'
      },
      {
        id: 5,
        ...
        menuType: '메인메뉴',
        page: '업무관리',
        pageCode: 'TASK-24-007',
        title: '데이터베이스 최적화'
      },
      {
        id: 6,
        dateTime: '2024-12-10 11:45',
        team: 'IT팀',
        user: '이수진',
        action: '하드웨어 등록',
        target: 'HW-24-003',
        description: '새로운 서버 장비 신규 등록',
        menuType: 'IT메뉴',
        page: '하드웨어관리',
        pageCode: 'HW-24-003',
        title: '서버 장비 신규 등록'
      },
      {
        ...
        action: '보안사고 처리',
        target: 'SEC-24-001',
        description: '사내 보안사고 신고 및 초기 대응 완료',
        menuType: '보안메뉴',
        page: '보안사고관리',
        pageCode: 'SEC-24-001',
        title: '사내 보안사고 신고'
    ...
    // 시간순으로 내림차순 정렬 (최신순)
    return combinedLogs.sort((a, b) => new Date(b.dateTime).getTime() - new Date(a.dateTime).getTime());
  };

  // 기존 changeLogs (현재 페이지의 로컬 변경로그)
  const [changeLogs, setChangeLogs] = useState<ChangeLog[]>([]);

  // 전체 변경로그 (모든 메뉴 통합)
  const [allChangeLogs, setAllChangeLogs] = useState<ChangeLog[]>([]);

  // 컴포넌트 마운트시 전체 변경로그 로드
  useEffect(() => {
    setAllChangeLogs(getAllChangeLogs());
  }, []);
