...
import CustomerSecurityDataTable from 'views/apps/CustomerSecurityDataTable';
...
function KanbanView({ selectedDepartment, selectedStatus, selectedYear }: KanbanViewProps) {
  ...
  // 데이터 필터링
  const filteredData = customerSecurityData.filter((inspection) => {
    // 연도 필터
    if (selectedYear !== '전체') {
...
function MonthlyScheduleView({ selectedDepartment, selectedStatus, selectedYear }: MonthlyScheduleViewProps) {
  ...
  // 월별로 데이터 그룹화
  const monthlyData: { [key: number]: CustomerSecurityRecord[] } = {};
...
export default function CustomerSecurityManagement() {
  const theme = useTheme();
  ...
  return (
    ...
    >
      ...
      <TabPanel value={value} index={0}>
        {/* 데이터 탭 - 테이블 */}
        <Box sx={{ p: 3 }}>
          <CustomerSecurityDataTable selectedDepartment={selectedDepartment} selectedStatus={selectedStatus} selectedYear={selectedYear} />
        </Box>
      </TabPanel>
