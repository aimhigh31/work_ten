...
export async function GET(request: NextRequest) {
  try {
    ...
    const codePattern = `MAIN-EDUCATION-${currentYear}-%`;
    ...
    if (data && data.length > 0) {
      ...
      const match = latestCode.match(/^MAIN-EDUCATION-(\d{2})-(\d{3})$/);
      if (match) {
    ...
    const newCode = `MAIN-EDUCATION-${currentYear}-${nextSequence.toString().padStart(3, '0')}`;
