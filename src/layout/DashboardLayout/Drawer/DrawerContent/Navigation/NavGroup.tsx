...
import { useGetMenuMaster, handlerGroupCollapse } from 'api/menu';
import { matchPath } from 'utils/matchPath';
import { MenuOrientation } from 'config';
import useConfig from 'hooks/useConfig';

// assets
import { More2, ArrowDown2, ArrowUp2 } from '@wandersonalwes/iconsax-react';
...
export default function NavGroup({
  ...
  const collapsedGroups = menuMaster.collapsedGroups || [];
  const isCollapsed = collapsedGroups.includes(item.id || '');
          ...
          subheader={
            <>
              {item.title ? (
                drawerOpen &&
                menuCaption && (
                  <Box sx={{ pl: 3, mb: 1.5, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    <Box>
                      <Typography
                        variant="h5"
                        sx={(theme) => ({
                    ...
                    <IconButton size="small" color="secondary" onClick={() => handlerGroupCollapse(item.id || '')} sx={{ mr: 1.5 }}>
                      {isCollapsed ? <ArrowDown2 size={16} /> : <ArrowUp2 size={16} />}
                    </IconButton>
                  </Box>
                )
              ) : (
