...
export default function NavItem({ item, level, isParents = false, setSelectedID }: Props) {
  ...
  const itemHandler = async () => {
    try {
      ...
      if (downLG) {
        await Promise.resolve(handlerDrawerOpen(false));
      }

      if (isParents && setSelectedID) {
        await Promise.resolve(setSelectedID());
      }
    } catch (error) {
      console.error('NavItem 핸들러 에러:', error);
    }
  };
          ...
          <ListItemButton
            ...
            onClick={async () => {
              try {
                ...
                console.error('NavItem onClick 에러:', error);
              }
            }}
          >
            {itemIcon && (
        ...
        <ListItemButton
          ...
          onClick={async () => {
            try {
              ...
              console.error('NavItem horizontal onClick 에러:', error);
            }
          }}
        >
          {itemIcon && (
