...
export default function NavCollapse({ menu, level, parentId, setSelectedItems, selectedItems, setSelectedLevel, selectedLevel }: Props) {
  ...
  const handleClick = async (event: ListItemClick, isRedirect: boolean) => {
    try {
      setAnchorEl(null);
      ...
      if (drawerOpen) {
        ...
        if (menu.url && isRedirect) {
          await Promise.resolve(router.push(`${menu.url}`));
        }
      } else {
        setAnchorEl(event?.currentTarget);
      }
    } catch (error) {
      console.error('NavCollapse handleClick 에러:', error);
    }
  };

  const handlerIconLink = async () => {
    try {
      if (!drawerOpen) {
        if (menu.url) {
          await Promise.resolve(router.push(`${menu.url}`));
      ...
      console.error('NavCollapse handlerIconLink 에러:', error);
    }
  };
          ...
          <ListItemButton
            ...
            {...(!drawerOpen && {
              onMouseEnter: async (e) => {
                try {
                  ...
                  console.error('NavCollapse onMouseEnter 에러:', error);
                }
              },
            ...
            onClick={async (e) => {
              try {
                ...
                console.error('NavCollapse onClick 에러:', error);
              }
            }}
            sx={(theme) => ({
          ...
          >
            {menuIcon && (
              <ListItemIcon
                onClick={async () => {
                  try {
                    await handlerIconLink();
                  } catch (error) {
                    console.error('NavCollapse icon onClick 에러:', error);
                  }
                }}
                sx={(theme) => ({
                  minWidth: 38,
                ...
                <IconButton
                  onClick={async (event) => {
                    try {
                      ...
                      console.error('NavCollapse IconButton onClick 에러:', error);
                    }
                  }}
                  color="secondary"
                  variant="outlined"
          ...
          <Box
            onClick={async () => {
              try {
                ...
                console.error('NavCollapse Box onClick 에러:', error);
              }
            }}
            sx={FlexBox}
          >
