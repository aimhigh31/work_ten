...
async function checkGroup015() {
  console.log('🔍 GROUP015 마스터코드 데이터 확인...');
  ...
  try {
    ...
    if (data && data.length > 0) {
      ...
      console.log(`📋 GROUP015 서브코드 목록 (${subcodes.length}개):`);
      subcodes.forEach(sub => {
        console.log(`  • ${sub.subcode_name} (${sub.subcode})`);
      });
    } else {
      console.log('⚠️ GROUP015 데이터가 없습니다. 샘플 데이터를 생성하시겠습니까?');
    }
